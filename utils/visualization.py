...
class WriterTensorboardX():
    def __init__(self, writer_dir, logger, enable):
        ...
        self.tensorboard_writer_ftns = ['add_scalar', 'add_scalars', 'add_image', 'add_figure', 'add_audio', 'add_text', 'add_histogram', 'add_pr_curve', 'add_embedding']
    ...
    def __getattr__(self, name):
        ...
        if name in self.tensorboard_writer_ftns:
            ...
            if name == 'add_pr_curve':
                def wrapper(tag, *args, **kwargs):
                    if add_data is not None:
                        add_data('{}/{}'.format(self.mode, tag), global_step=self.step, *args, **kwargs)
                return wrapper
